...
function Portrait() {
  ...
  const src = `${import.meta.env.BASE_URL}me.jpg`

  return (
    <div className="aspect-[3/4] w-full rounded-[40px] overflow-hidden bg-gradient-to-b from-[#2a2a2e] to-[#0c0c0c] border border-white/10 flex items-center justify-center">
      {!imgError ? (
        <img src={src} alt={content.name} className="w-full h-full object-cover" onError={() => setImgError(true)} />
      ) : (
        <span className="text-white/20 text-xs uppercase tracking-widest px-4 text-center">Add me.jpg to /public</span>
      )}
    </div>
  )
}
